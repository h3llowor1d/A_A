<?php
    ...
    class QQ {
        ...
        public $authorize = "https://graph.qq.com/oauth2.0/authorize";
         ...
         * @ignore
         */
        function authorizeURL() { return $this->authorize; }
        ...
        function getOpenid($url,$params){
            $response = $this->oAuthRequest($url, 'GET', $params);
            return $this->parse($response,'url');
        }

        function getUserInfo($params){
            $response = $this->oAuthRequest("https://graph.qq.com/user/get_user_info", 'GET', $params);
        ...
        function parse($response,$format){
            if($format === 'url'){
                if(strpos($response, "callback") !== false){
                    ...
                    parse_str($response,$token);//url把参数转换为变量 并设置到当前作用域 第二个参数作为接受URL参数的数组
                }
